...
[English](../../en/agent-observability/agentsight.md)
...
- [快速开始](agentsight/QUICKSTART.md)——安装、启动追踪、产生第一条会话、打开 Dashboard。
- [Dashboard 指南](agentsight/dashboard.md)——令牌访问方式与逐页说明。
- [CLI 参考](agentsight/cli-reference.md)——所有命令、参数与真实输出。
- [配置](agentsight/configuration.md)——配置文件、功能开关、Agent 发现规则。

## 你能用它做什么

| 目标 | 阅读 |
|---|---|
| 看清有哪些 Agent 在跑、各花了多少 Token | [快速开始](agentsight/QUICKSTART.md) |
| 查清某个 Agent 任务为什么卡住、失败或空转 | [中断检测](agentsight/interruption-detection.md) |
| 逐步回放一次会话（提示词、工具调用、结果） | [Dashboard 指南](agentsight/dashboard.md#轨迹查看) |
| 让 AgentSight 认识一个它还不识别的 Agent | [配置](agentsight/configuration.md#agent-发现规则) |
| 以服务、容器或 macOS 形态运行 | [部署](agentsight/deployment.md) |
| 从脚本、Prometheus 或其他系统取数据 | [数据与存储](agentsight/data-and-storage.md) |
| 与 Tokenless、agent-sec-core、cosh 配合 | [集成](agentsight/integrations.md) |
| 解决“没数据”“401”“端口打不开” | [排查](agentsight/troubleshooting.md) |
...
| 磁盘 | `/var/log/sysak/.agentsight` 下几百 MB，有容量上限，见[数据与存储](agentsight/data-and-storage.md) |
...
完整流程与真实输出见[快速开始](agentsight/QUICKSTART.md)。

## 参考页面

| 页面 | 内容 |
|---|---|
| [快速开始](agentsight/QUICKSTART.md) | 安装、验证采集、第一次打开 Dashboard |
| [Dashboard 指南](agentsight/dashboard.md) | 认证方式与所有 Dashboard 页面 |
| [CLI 参考](agentsight/cli-reference.md) | `trace`、`serve`、`dashboard`、`token`、`audit`、`discover`、`metrics`、`summary`、`interruption`、`skill-metrics` |
| [配置](agentsight/configuration.md) | `config.json` 结构、功能开关、运行时上限、发现规则 |
| [中断检测](agentsight/interruption-detection.md) | 18 种中断类型与排查流程 |
| [部署](agentsight/deployment.md) | systemd、前台、容器/Sidecar、macOS、升级、卸载 |
| [数据与存储](agentsight/data-and-storage.md) | 数据库、保留策略、HTTP API、Prometheus、ATIF 导出 |
| [集成](agentsight/integrations.md) | Tokenless、agent-sec-core、enforcer、cosh、Prometheus |
| [排查](agentsight/troubleshooting.md) | 没数据、401、端口不通、数据库增长 |
