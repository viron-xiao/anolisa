...
fn audit_chain_creates_one_open_unblocked_case_with_ordered_evidence() {
    ...
    let cases = store.list_cases(100, 0, None).expect("case list should load");
...
fn allowed_containment_decision_appends_without_claiming_a_block() {
    let (store, event_ids) = ingest_mock_chain(PolicyMode::Audit);
    let case_id = store.list_cases(10, 0, None).expect("case list should load")[0].case_id;
    let binding_id = Uuid::new_v4();
    ...
    assert_eq!(store.list_cases(10, 0, None).expect("cases should load").len(), 1);
}
